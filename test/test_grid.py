...
class GridTest(unittest.TestCase):
    ...
    def test_add_entity(self):
        grid = Grid(110, 100, 10)
        entity = Entity(grid, pg.Vector2(2, 2), 1, None)
    ...
    def test_move_entity(self):
        grid = Grid(110, 100, 10)
        entity = Entity(grid, pg.Vector2(0, 1), 1, None)
    ...
    def test_cell_edges(self):
        ...
        grid = Grid(10, 2, 1)
        entity = Entity(grid, pg.Vector2(0.999, 0.999), 1, None)

        self.assertEqual(entity._cell.x, 0)
        self.assertEqual(entity._cell.y, 0)

        grid = Grid(10, 2, 1)
        entity = Entity(grid, pg.Vector2(0, 0), 1, None)

        self.assertEqual(entity._cell.x, 0)
        self.assertEqual(entity._cell.y, 0)

        grid = Grid(10, 2, 1)
        entity = Entity(grid, pg.Vector2(1, 1), 1, None)
    ...
    def test_distance(self):
        grid = Grid(10, 10, 1)
        e0 = Entity(grid, pg.Vector2(4.5, 4.5), 1, None)
        e1 = Entity(grid, pg.Vector2(4.6, 4.6), 2, None)
        e2 = Entity(grid, pg.Vector2(5.1, 4.5), 3, None)
        e3 = Entity(grid, pg.Vector2(10, 0.2), 4, None)

        # Happy path, in range of 1
        nearby = list(map(strip_tuple, e1.get_nearby_entities(1)))
        nearby.sort(key=order_entities)
        self.assertListEqual(nearby, [e0, e2])

        # Not in range of anything
        # Also on the edge of the grid!
        nearby = list(map(strip_tuple, e3.get_nearby_entities(1)))
        nearby.sort(key=order_entities)
    ...
    def test_distance_on_edge(self):
        ...
        grid = Grid(10, 10, 1)
        e0 = Entity(grid, pg.Vector2(0.5, 0.5), 1, None)
        e1 = Entity(grid, pg.Vector2(1.5, 1.5), 2, None)
        e2 = Entity(grid, pg.Vector2(1.5, 0.5), 3, None)
        e3 = Entity(grid, pg.Vector2(0.5, 1.5), 4, None)

        nearby = list(map(strip_tuple, e0.get_nearby_entities(1)))
        nearby.sort(key=order_entities)
        self.assertListEqual(nearby, [e2, e3])

        # Entities clustered around bottom right
        grid = Grid(10, 10, 1)
        e0 = Entity(grid, pg.Vector2(10, 10), 1, None)
        e1 = Entity(grid, pg.Vector2(9, 9), 2, None)
        e2 = Entity(grid, pg.Vector2(9, 10), 3, None)
        e3 = Entity(grid, pg.Vector2(10, 9), 4, None)
    ...
    def test_cross_cell_search(self):
        grid = Grid(10, 10, 1)
        e0 = Entity(grid, pg.Vector2(5.1, 5.1), 1, None)
        e1 = Entity(grid, pg.Vector2(4, 5.1), 2, None)
        e2 = Entity(grid, pg.Vector2(3.9, 5.1), 3, None)
        # Verified 6.2 does not work because of floating point innaccuracies
        e3 = Entity(grid, pg.Vector2(6.19999, 5.1), 4, None)
        e4 = Entity(grid, pg.Vector2(6.3, 5.1), 5, None)

        nearby = list(map(strip_tuple, e0.get_nearby_entities(1.1)))
        nearby.sort(key=order_entities)
    ...
    def test_cross_grid_search(self):
        ...
        e2 = Entity(grid, pg.Vector2(9, 10), 3, None)
        e3 = Entity(grid, pg.Vector2(10, 9), 4, None)
        e4 = Entity(grid, pg.Vector2(5, 2), 5, None)
        e5 = Entity(grid, pg.Vector2(2, 4), 6, None)
        e6 = Entity(grid, pg.Vector2(1, 10), 7, None)
        e7 = Entity(grid, pg.Vector2(2.2, 4.4), 8, None)

        nearby = list(map(strip_tuple, grid.get_nearby_entities(pg.Vector2(5, 4.9), 100)))
        nearby.sort(key=order_entities)
