...
from src.soup.engine.builtin.component import Velocity
...
class TestECS(unittest.TestCase):
    ...
    def test_remove_entity(self):
        manager = ecs.ECS(100, 100, 10)
        e1 = manager.add_entity(pos=pg.Vector2(1,1)).attach(Velocity(pg.Vector2(-4, -3)))
        e2 = manager.add_entity(pos=pg.Vector2(2,2)).attach(Velocity(pg.Vector2(-4, -3)))
        e3 = manager.add_entity(pos=pg.Vector2(3,3)).attach(Velocity(pg.Vector2(-4, -3)))
        e4 = manager.add_entity(pos=pg.Vector2(4,4)).attach(Velocity(pg.Vector2(-4, -3)))

        self.assertTrue(e1.eid in manager.eindex)
        self.assertTrue(e1 in manager.cindex[Velocity.c_type_id])
        self.assertTrue(e1 in list(map(strip_tuple, e2.get_nearby_entities(10))))
        ...
        self.assertFalse(e1 in list(map(strip_tuple, e2.get_nearby_entities(10))))