...
def weight_mutate(genotype: Genotype) -> None:
...
def split_connection_mutate(genotype: Genotype) -> None:
    ...
    # Order values of the nodes the connection we are splitting attached to
    sourceOrder = genotype.node_genes[originalConnection.source].order
    toOrder = genotype.node_genes[originalConnection.to].order

    newOrder = (sourceOrder + toOrder) / 2
    newNode = NodeGene(NodeType.HIDDEN, newOrder, bias=0)

    genotype.add_node(newNode)


def add_connection_mutate(genotype: Genotype) -> None:
